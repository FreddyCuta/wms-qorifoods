...
function statusBadge(estado) {
  ...
  if (estado === 'parcial') return <Badge color="blue">Atendido parcialmente</Badge>
          ...
          <tbody className="divide-y divide-border">
            {requirements.map((r) => (
              <tr key={r.id} className="transition-colors hover:bg-accent/40">
                ...
                <td className="px-4 py-3 text-muted-foreground">{r.fechaRegistro}</td>
                <td className="px-4 py-3 text-foreground">{r.registradoPor}</td>
                <td className="px-4 py-3 text-foreground">
                  {r.insumos.length} {r.insumos.length === 1 ? 'insumo' : 'insumos'}
                </td>
                ...
                <td className="px-4 py-3 text-right">
                  {r.estado === 'atendido' ? (
                    ...
                    <ActionButton
                      ...
                      onClick={() => navigate(`/requerimientos/${r.id}/atender`)}
                    >
                      Atender
                    </ActionButton>
                  ) : (
