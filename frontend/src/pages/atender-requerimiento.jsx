...
export default function AtenderRequerimientoPage() {
  const { id } = useParams();
  const { requirements, inventory, currentUser, attendRequirement, addToast } =
    useApp();
  ...
  const salidas = req.insumos.map((item) => {
    ...
    const esInsuficiente =
      cantidad > 0 && yaAtendido + cantidad < item.cantidad;
  ...
  const todasComplete = salidas.every(
    (s) => s.yaAtendido + s.salidaQty >= s.cantidad,
  );
  const algunaFaltante = salidas.some(
    (s) => s.salidaQty > 0 && s.yaAtendido + s.salidaQty < s.cantidad,
  );
  ...
  function confirmSalida() {
    ...
    addToast(
      `Requerimiento ${req.numero} ${status}. Salidas de insumos registradas.`,
    );
    ...
    <AppShell title="Atender Requerimiento" allowedRoles={["operario"]}>
      ...
      <div className="flex items-start justify-between">
        ...
        {req.atenciones?.length > 0 && (
          <Badge color="amber">
            {req.atenciones.length} atención(es) previa(s)
          </Badge>
        )}
      </div>
      ...
      <div className="mt-5 rounded-lg border border-border bg-card p-4">
        <h2 className="text-sm font-semibold text-foreground mb-4">
          Insumos solicitados para producción
        </h2>
        <div className="flex flex-col gap-4">
          {salidas.map((item) => {
            const fefo = sugerirLoteFefo(inventory, item.key) || {
              lote: "N/A",
              vence: "N/A",
              ubicacion: "N/A",
            };
            ...
            return (
              <div
                key={item.key}
                className={`rounded-lg border ${item.completadoPrevio ? "border-success/40 bg-success/5" : "border-border bg-muted"} p-4`}
              >
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Lote sugerido (FEFO): {fefo.lote} | Vence: {fefo.vence} |{" "}
                      {fefo.ubicacion}
                    </p>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-medium text-foreground">
                      {qty(stockDisponible[item.key] || 0, item.unidad)}
                    </div>
                ...
                {item.yaAtendido > 0 && (
                  <div className="mb-2 flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <span className="font-medium text-foreground">
                      {qty(item.yaAtendido, item.unidad)}
                    </span>
                    {item.pendiente > 0 && (
                      <span>
                        · Pendiente:{" "}
                        <span className="font-medium text-warning">
                          {qty(item.pendiente, item.unidad)}
                        </span>
                      </span>
                    )}
                  </div>
                ...
                {item.completadoPrevio ? (
                  ...
                  <>
                    ...
                    <Field
                      label={`Cantidad a despachar ahora (máx: ${qty(item.pendiente, item.unidad)})`}
                    >
                      <div className="flex items-end gap-3">
                        <div className="flex-1 relative">
                          <TextInput
                            ...
                            onChange={(e) =>
                              handleSalidaChange(item.key, e.target.value)
                            }
                            placeholder="0"
                            className="pr-12"
                          />
        ...
        <div className="mt-4 flex items-start gap-3 rounded-lg border border-warning/30 bg-warning/8 px-4 py-3">
          ...
          <div className="text-sm text-warning">
            <p className="font-medium">
              Stock insuficiente para completar todo
            </p>
            <p className="text-xs mt-1">
              Se despachará lo disponible. El requerimiento quedará como parcial
              hasta que llegue más stock y se complete la atención.
            </p>
          </div>
        </div>
      )}
      ...
      >
        <div className="space-y-4">
          {todasComplete ? (
            <p className="text-sm text-foreground">
              Todas las cantidades solicitadas serán despachadas. El
              requerimiento se marcará como completado.
            </p>
          ) : (
            <div className="flex items-start gap-3 rounded-lg border border-warning/30 bg-warning/8 px-3 py-2">
              <AlertTriangle className="mt-0.5 size-4 shrink-0 text-warning" />
              <p className="text-xs text-warning">
                Este requerimiento quedará como parcialmente atendido. Podrá
                completarse cuando haya más stock disponible.
              </p>
            </div>
          )}
          <div className="space-y-2 text-sm">
            <h4 className="font-medium text-foreground">
              Resumen de esta atención:
            </h4>
            {salidas
              .filter((s) => s.salidaQty > 0)
              .map((item) => (
                <div
                  key={item.key}
                ...
                >
                  ...
                  <span className="font-medium">
                    {qty(item.salidaQty, item.unidad)}
                  </span>
                </div>
              ))}
          </div>
          {salidas.some((s) => s.yaAtendido > 0) && (
            <div className="space-y-2 text-sm">
              <h4 className="font-medium text-foreground">
                Acumulado total (previa + esta):
              </h4>
              {salidas
                .filter((s) => s.yaAtendido + s.salidaQty > 0)
                .map((item) => {
        ...
        <ModalFooter>
          <ActionButton variant="ghost" onClick={() => setConfirmOpen(false)}>
            Cancelar
          </ActionButton>
